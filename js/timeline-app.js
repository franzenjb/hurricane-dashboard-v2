function hurricaneApp() {
    return {
        ...
        updateStateComparison() {
            // Get storms for selected states
            ...
            const traces = this.selectedStatesComparison.map(state => {
                const stateData = this.allStorms.filter(s => s.state === state);
                return {
                    x: ['Cat 1', 'Cat 2', 'Cat 3', 'Cat 4', 'Cat 5'],
                    y: [1, 2, 3, 4, 5].map(cat => 
                        stateData.filter(s => s.category === cat).length
                    ),
            ...
            Plotly.newPlot('state-comparison-chart', traces, layout, {responsive: true});
        },
